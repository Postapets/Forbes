...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Создаем адаптер, получая массив объектов Person
        PersonAdapter adapter = new PersonAdapter(this, makeForbes());
        //Создаем объект листвью и привязываем к основному листу со списком
    ...
    //Метод создает и возвращает массив объектов класса Person уже заполненный нужными данными
    Person[] makeForbes(){
        ...
        Person[] arr = new Person[money.length];
        ...
        String[] forbes = res.getStringArray(R.array.forbes);

        for (int i =0; i<len;i++){
            Person pers = new Person(forbes[i], money[i], flags[i]);
            arr[i]=pers;
        }
        return arr;
    }
